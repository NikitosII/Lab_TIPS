...
async def check_with_ai(content: str) -> str:
    """Проверка работы через OpenRouter с разными моделями"""
    
    prompt = f"""
Тщательно проанализируй лабораторную работу и дай развернутую оценку СТРОГО в указанном формате.
...
[2-3 предложения о теме и содержании работы]

**Оценка по критериям:**

1. Качество решения (X/30):
[Конкретные замечания по реализации, обоснование оценки]

2. Полнота реализации (Y/30):  
[Что выполнено, что пропущено, соответствие заданию]

3. Документация и комментарии (Z/20):
[Качество описания, пояснений, комментариев в коде]

4. Оформление и структура (W/20):
[Логичность изложения, оформление графиков, формул, списков]
...
4. [Конкретная рекомендация 4]

**Итоговая оценка: [СУММА_БАЛЛОВ]/100 баллов**

Требования:
- Будь конкретен в оценках и обоснованиях
- Указывай реальные баллы за каждый критерий
- Рекомендации должны быть практическими и выполнимыми
- Сумма баллов по критериям должна равняться итоговой оценке

"""
    
    try:
        response = openrouter.chat.completions.create(
            model = "mistralai/mistral-7b-instruct:free",  # №1
            # model = "google/gemma-2-9b-it:free", №2
            # model="microsoft/phi-3-medium-128k-instruct:free", №3
            messages=[
